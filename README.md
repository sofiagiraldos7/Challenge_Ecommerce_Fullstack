...
## <img src="https://flagcdn.com/w40/co.png" width="30"> Plantilla base (estado actual)

- El proyecto corre **sin configuración de Firebase**.
- La base está implementada con **mockdata + localStorage**:
  - Registro e inicio de sesión simulado
  - Galería con búsqueda y paginación
  - Carrito persistente
  - Checkout simulado
- Para iniciar rápido puedes usar usuarios de `src/mockdata/mock_users.js`, por ejemplo:
  - `andres@example.com / password123`
  - `beatriz@example.com / securePass456`
...
7. Ejecuta el proyecto:
   ...
   npm run dev
   ```
8. Listo: no necesitas `firebase.config.js` para correr la plantilla base.
...
4. Initialize Database (Seeder)
   ...
   - This step is **optional** and only applies if you decide to use Firebase (bonus).
   - Run the following command to push mock products into your Firestore (ensure `src/firebase/firebase.config.js` exists with real credentials):
   ```bash
   npm run seed
   ```
